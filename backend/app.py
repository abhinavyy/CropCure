...
# --- Lazy-loaded ML models (only loaded on first /predict request) ---
...
def _load_models():
    """Load ML models lazily to avoid OOM at startup on free tier."""
    global _models_loaded, leaf_model, leaf_model_loaded, leaf_transform, leaf_device
    global disease_model, disease_model_loaded, disease_transform, disease_device
    ...
    leaf_model_loaded = False
    for path in leaf_model_paths:
        if os.path.exists(path):
            try:
                leaf_model.load_state_dict(torch.load(path, map_location=leaf_device))
                leaf_model.eval()
                leaf_model_loaded = True
                print(f"Leaf verification model loaded from: {path}")
                break
            except Exception as e:
                print(f"Failed to load leaf model from {path}: {e}")
    ...
    disease_model_loaded = False
    for path in disease_model_paths:
        if path and os.path.exists(path):
            try:
                disease_model.load_state_dict(torch.load(path, map_location=disease_device))
                ...
                print(f"Failed to load disease model from {path}: {e}")
...
def verify_leaf_image(image_path):
    ...
    try:
        ...
        print(f"DEBUG: Interpretation (conf<0.5=leaf): {is_leaf_interpretation2}")
...
def predict():
    # Lazy-load models on first predict request
    _load_models()
    ...
    try:
        from plant_disease_classifier import predict_image
